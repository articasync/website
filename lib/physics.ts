...
/**
 * Computes the physiological Maximum Lactate Steady State (MLSS) power in Watts
 * based on mitochondrial density, fiber type composition, and lactate transporters.
 */
export function calculateBaselineMLSS(hardware: Hardware): number {
  ...
  return 150 + 300 * normMito * normFiber1 * normMct1 - 50 * fiberType2 * normMct4;
}

/**
 * Fast headless simulation to test whether an athlete can sustain a given wattage
 * for a target duration without triggering physiological exhaustion failure.
 */
function canSustainPower(
  hardware: Hardware,
  watts: number,
  targetSeconds: number
): boolean {
  const MLSS = calculateBaselineMLSS(hardware);
  const normMito = hardware.mitoDensity / 15.0;
  const normMct1 = hardware.mct1Density / 300.0;
  const normMct4 = hardware.mct4Density / 300.0;
  ...
  for (let s = 0; s < targetSeconds; s++) {
    ...
    const flux_mct1 =
      watts < MLSS
        ? K_OX * normMct1 * normMito * la_blood * (MLSS - watts)
        : 0.0;
    ...
    // Metabolic heat generation & temp
    const heat_gen = watts * (1 / current_gme - 1);
    const d_temp =
      heat_gen * 0.0001 - normCooling * 0.05 * (core_temp - 37.0);

    // Fluid loss & SV
    const d_fluid = normSweat * 0.001 * Math.max(0, core_temp - 37.0);
    fluid_loss += d_fluid;
    sv = Math.max(
      40,
    ...
    // Failure criteria
    if (la_muscle > 20.0 || pcr2 <= 0 || glycogen <= 0 || pi > 1.5) {
      return false; // Blew up before reaching target duration
    }
  }

  return true; // Survived the duration
}

/**
 * Uses binary search over physiological ODE simulations to determine the exact
 * maximum sustainable power in Watts for a specific duration in seconds.
 */
export function findMaxPowerForDuration(
  hardware: Hardware,
  ...
  // 15 iterations of binary search yields < 0.1W precision across [50, 2500]W
  for (let i = 0; i < 15; i++) {
    const mid = (low + high) / 2;
    if (canSustainPower(hardware, mid, durationSeconds)) {
      best = mid;
      low = mid;
    } else {
      high = mid;
...
export function runSimulation(
  ...
  for (const block of workout) {
    ...
    for (let s = 0; s < block.durationSeconds; s++) {
      ...
      // Failure check
      const blown_up =
        la_muscle > 20.0 || pcr2 <= 0 || glycogen <= 0 || pi > 1.5;
